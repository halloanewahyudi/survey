<template>
      <div class="rating-description italic font-semibold lg:hidden block">
        {{ ratingDescription }}
      </div>
    <div class="rating flex gap-6 items-center transition-all duration-150 ease-in-out" :class="ratingClass">
      <span
      class="w-9 h-9 rounded-full bg-neutral-50 border-secondary border-2 cursor-pointer"
        v-for="number in 5"
        :key="number"
      ...
      <div class="rating-description italic font-semibold hidden lg:block">
        {{ ratingDescription }}
      </div>
    </div>
  </template>
