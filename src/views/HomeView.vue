...
<template>
  ...
  <div v-else class="main" :class="{'tampil' : tampil  } ">
    <header class="survey container mx-auto bg-primary text-white flex flex-col gap-4 pb-6">
    ...
    <div class="bg-secondary container mx-auto">
      <form @submit.prevent="submitForm" class="flex flex-col gap-2">
        <div class="rating-field grid grid-cols-1 lg:grid-cols-2 items-center pt-4 lg:pt-10">
          <div class="rt-text bg-primary text-white">
            ...
            <p class="font-semibold">
              Nilai Kepuasan anda terhadap akses lokasi Kawasan Industri Dumai, (waktu tempuh, akses transportasi umum
              dan tempat parkir)?
            </p>
            <p>
        ...
        <button type="submit"
          class="bg-primary text-white py-3 px-9 max-w-max rounded-lg hover:scale-105 duration-300 m-6">Selesai</button>
      </form>
    </div>
  </div> <!-- endmain -->
</template>
